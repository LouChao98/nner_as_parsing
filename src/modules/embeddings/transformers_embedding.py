...
from src.utils.alg import pad as simple_pad

from ..blocks import ScalarMix
...
class TransformersAdaptor(EmbeddingAdaptor):
    ...
    def trigger(cls, target_string):
        return target_string == 'src.modules.embeddings.TransformersEmbedding'
...
def enable_transformers_embedding(datasets, tokenizer, fix_len=20):
    def get_subwords(_words):
        ...
        return simple_pad(sws, tokenizer.pad_token_id).numpy()
...
class TransformersEmbedding(nn.Module):
    ...
    def forward(self, subwords):
        ...
        if self.pooling == 'first':
            ...
            embed = embed.gather(2, (bert_lens - 1).unsqueeze(-1) \
                                                   .repeat(1, 1, self.hidden_size).unsqueeze(2)).squeeze(2)
        else:
