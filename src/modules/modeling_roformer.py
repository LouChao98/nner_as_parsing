...
from transformers.modeling_utils import (PreTrainedModel, SequenceSummary, apply_chunking_to_forward,
...
ROFORMER_PRETRAINED_MODEL_ARCHIVE_LIST = [
    'junnyu/roformer_chinese_small', 'junnyu/roformer_chinese_base', 'junnyu/roformer_chinese_char_small',
    'junnyu/roformer_chinese_char_base', 'junnyu/roformer_small_discriminator', 'junnyu/roformer_small_generator'
...
class RoFormerSinusoidalPositionalEmbedding(nn.Embedding):
    ...
    def _init_weight(out: nn.Parameter):
        ...
        """
        n_pos, dim = out.shape
        position_enc = np.array([[pos / np.power(10000, 2 * (j // 2) / dim) for j in range(dim)]
...
class RoFormerSelfAttention(nn.Module):
            ...
            if sinusoidal_pos is not None:
                if self.rotary_value:
                    (
                        ...
                        key_layer,
                        value_layer,
                    ) = self.apply_rotary_position_embeddings(sinusoidal_pos, query_layer, key_layer, value_layer)
                else:
                    query_layer, key_layer = self.apply_rotary_position_embeddings(sinusoidal_pos, query_layer,
                                                                                   key_layer)
        if self.is_decoder:
    ...
    def apply_rotary_position_embeddings(sinusoidal_pos, query_layer, key_layer, value_layer=None):
        ...
        rotate_half_query_layer = torch.stack([-query_layer[..., 1::2], query_layer[..., ::2]],
        ...
        if value_layer is not None:
            ...
            rotate_half_value_layer = torch.stack([-value_layer[..., 1::2], value_layer[..., ::2]],
                                                  dim=-1).reshape_as(value_layer)
...
class RoFormerForMaskedLM(RoFormerPreTrainedModel):
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning('If you want to use `RoFormerForMaskedLM` make sure `config.is_decoder=False` for '
