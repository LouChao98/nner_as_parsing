...
class Alignment(_Struct):
    def __init__(self, semiring=LogSemiring, sparse_rounds=3, max_gap=None, local=False):
    ...
    def _dp_scan(self, log_potentials, lengths=None, force_grad=False):
        ...
        ssize = semiring.size()
        log_potentials, batch, N, M, lengths = self._check_potentials(log_potentials, lengths)
        ...
        charta[0] = self._make_chart(1, (batch, bin_N, 1, bin_N, LOC, LOC, 3), log_potentials, force_grad)[0]
        charta[1] = self._make_chart(1, (batch, bin_N // 2, 3, bin_N, LOC, LOC, 3), log_potentials, force_grad)[0]
        ...
        for b in range(lengths.shape[0]):
            ...
            l = lengths[b]
            charta[0][:, b, rot_x[:l], 0, rot_y[:l], :, :, :] = log_potentials[:, b, :l, :, None, None]

            # Create finalizing paths.
            point = (l + M) // 2

            charta[1][:, b, point:, 1, ind, :, :, Mid] = semiring.one_(charta[1][:, b, point:, 1, ind, :, :, Mid])
        ...
        def merge(x):
            inner = x.shape[-1]
            width = (inner - 1) // 2
            left = x[:, :, 0::2, Open, :].view(ssize, batch, -1, 1, LOC, 3, bin_N, inner)
            right = x[:, :, 1::2, :, Open].view(ssize, batch, -1, LOC, 1, 1, 3, bin_N, inner)
        ...
        if self.local:
            ...
            v = chart[..., 0, Open, Open, Mid, N - 1, M - N + ((chart.shape[-1] - 1) // 2)]
