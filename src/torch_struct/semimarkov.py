...
class SemiMarkov(_Struct):
    ...
    def _check_potentials(self, edge, lengths=None):
        batch, N_1, K, C, C2 = self._get_dimension_and_requires_grad(edge)
    ...
    def _dp(self, log_potentials, lengths=None, force_grad=False, cache=True):
        ...
        c[:, :, :K - 1, 0] = semiring.sum(torch.stack([c.data[:, :, :K - 1, 0], lp[:, :, 1:K]], dim=-1))
        ...
        # Order n, n-1
        chart = (init.permute(0, 1, 2, 3, 5, 4, 6).contiguous().view(-1, batch, bin_N, K_1 * C, K_1 * C))
    ...
    def enumerate(self, edge):
        ...
        chains = {}
        chains[0] = [([(c, 0)], semiring.one_(torch.zeros(ssize, batch))) for c in range(C)]
