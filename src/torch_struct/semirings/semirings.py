...
class Semiring(ABC):
    ...
    * Semiring parsing :cite:`goodman1999semiring`
    ...
    @classmethod
    def convert(cls, potentials: Tensor) -> Tensor:
...
class KLDivergenceSemiring(Semiring):
    ...
    * Sample Selection for Statistical Grammar Induction :cite:`hwa2000samplesf`
    ...
    def sum(xs, dim=-1):
        ...
        sm_p = log_sm_p.exp()
        return torch.stack((part_p, part_q, sm_p.mul(xs[2] - log_sm_q + log_sm_p).sum(dim=d)))
...
class CrossEntropySemiring(Semiring):
    ...
    * Sample Selection for Statistical Grammar Induction :cite:`hwa2000samplesf`
    ...
    def sum(cls, xs, dim=-1):
        ...
        log_sm_q = xs[1] - part_q.unsqueeze(d)
        sm_p = log_sm_p.exp()
        return torch.stack((part_p, part_q, torch.sum((xs[2] - log_sm_q).mul(sm_p), dim=d)))
...
class EntropySemiring(Semiring):
    ...
    * Sample Selection for Statistical Grammar Induction :cite:`hwa2000samplesf`
    ...
    def sum(xs, dim=-1):
        ...
        sm = log_sm.exp()
        return torch.stack((part, torch.sum((xs[1] - log_sm).mul(sm), dim=d)))
