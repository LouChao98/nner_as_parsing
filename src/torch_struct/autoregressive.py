...
from .semirings import KMaxSemiring, MaxSemiring, TempMax
    ...
    * event_shape -> N x T sequence of choices
    ...
    def log_prob(self, value, sparse=False):
        ...
        if not sparse:
            ...
            value = ((value * torch.arange(n_classes).view(1, 1, n_classes)).sum(-1).long())
        else:
            sample, batch_shape, n_length = value.shape

        value = torch.cat(
            [
                torch.zeros(sample, batch_shape, 1, device=value.device).fill_(self.start_class).long(),
                value,
            ],
            dim=2,
        )
        value = unwrap(value)
        state = tuple((unwrap(i.unsqueeze(0).expand((sample, ) + i.shape)) for i in self.init))

        logits, _ = self.model(value, state)
        b2, n2, c2 = logits.shape
        assert ((b2 == sample * batch_shape) and (n2 == n_length + 1)
                and (c2 == self.n_classes)), 'Model should return logits of shape `batch x N x C` '
    ...
    def _beam_search(self, semiring, gumbel=False):
        ...
        def take(state, indices):
            return tuple((s.contiguous()[(
                indices * self.batch_shape[0] +
                torch.arange(self.batch_shape[0], device=self.device).unsqueeze(0)).contiguous().view(-1)]
                          for s in state))

        tokens = (torch.zeros((ssize * self.batch_shape[0])).long().fill_(self.start_class))
        state = tuple((unwrap(i.unsqueeze(0).expand((ssize, ) + i.shape)) for i in self.init))
        ...
        for t in range(0, self.n_length):
            ...
            b2, n2, c2 = logits.shape
            assert ((b2 == ssize * self.batch_shape[0]) and (n2 == 1)
                    and (c2 == self.n_classes)), 'Model should return logits of shape `batch x N x C` '
            for s in state:
                assert (s.shape[0] == ssize *
                        self.batch_shape[0]), 'Model should return state tuple with shapes `batch x ...` '
            logits = wrap(logits.squeeze(1), ssize)
            if gumbel:
                logits = logits + torch.distributions.Gumbel(0.0, 1.0).sample(logits.shape)
            if self.normalize:
        ...
        for k in range(v.shape[0]):
            obj = v[k].sum(dim=0)
            marg = torch.autograd.grad(obj, all_beams, create_graph=True, only_inputs=True, allow_unused=False)
    ...
    def sample(self, sample_shape=torch.Size()):
        ...
        state = tuple((unwrap(i.unsqueeze(0).expand((sample_shape, ) + i.shape)) for i in self.init))
        all_tokens = []
        tokens = (torch.zeros((sample_shape * self.batch_shape[0])).long().fill_(self.start_class))
        for t in range(0, self.n_length):
