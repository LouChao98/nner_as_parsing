...
class LinearChain(_Struct):
    ...
    def _check_potentials(self, edge, lengths=None):
        batch, N_1, C, C2 = self._get_dimension_and_requires_grad(edge)
    ...
    def _dp_scan(self, log_potentials, lengths=None, force_grad=False):
        ...
        ssize = semiring.size()
        log_potentials, batch, N, C, lengths = self._check_potentials(log_potentials, lengths)
    ...
    def to_parts(sequence, extra, lengths=None):
        ...
        labels = torch.zeros(batch, N - 1, C, C, dtype=torch.long)
        if lengths is None:
    ...
    def from_parts(edge):
        ...
        on = edge.cpu().nonzero(as_tuple=False)
    ...
    def _rand(min_n=2):
        ...
        N = torch.randint(min_n, 4, (1, )).item()
        C = torch.randint(2, 4, (1, )).item()
        return torch.rand(b, N, C, C), (b, (N + 1))
