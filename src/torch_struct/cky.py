...
class CKY(_Struct):
    def _dp(self, scores, lengths=None, force_grad=False, cache=True):
        ...
        span: List[Optional[Tensor]] = [None for _ in range(N)]
        v = (ssize, batch)
        term_use = terms + 0.0
    ...
    def marginals(self, scores, lengths=None, _autograd=True, _raw=False):
        ...
        Parameters:
            scores: a tuple with three elements:
                terms  b x n x T
        ...
        v, (term_use, rule_use, root_use, spans), alpha = self._dp(scores,
                                                                   lengths=lengths,
                                                                   force_grad=True,
                                                                   cache=not _raw)
    ...
    def score(self, potentials, parts, **kwargs):
        terms, rules, roots = potentials[:3]
        m_term, m_rule, m_root = parts[:3]
        b = m_term.shape[0]
        return (m_term.mul(terms).view(b, -1).sum(-1) + m_rule.mul(rules).view(b, -1).sum(-1) +
                        ...
                        for z in range(NT) if w != end - 1 else range(NT, NT + T):
                            for m1, y1 in enumerate(y, start, w):
                                for m2, z1 in enumerate(z, w, end):
                                    yield (
                                        semiring.times(semiring.times(m1, m2), rules[:, x, y, z]),
                                        [(x, start, w, end)] + y1 + z1,
                                    )
