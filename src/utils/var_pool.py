...
def _get_mask(seq_len, max_len):
    m = seq_len_to_mask(seq_len - 1, max_len)
    m[:, 0] = 0
    return m
...
class VarPool:
    def __init__(self, **kwargs):
        ...
        self.add_lazy('seq_len', 'real_len', lambda x: x - 2)
        self.add_lazy('real_len', 'num_token', lambda x: sum(x))
        self.add_lazy(['seq_len', 'max_len'], 'mask', _get_mask)
